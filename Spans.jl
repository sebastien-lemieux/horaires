...
export Span, expand, acatosyn, _conflict
...
struct Span # Add a row id to to reidentify the exact schedule that cause conflict
    s_id::Int
    imm::Symbol # Building id
    s::DateTime
    e::DateTime
end

Span(s_id::Int, imm::Symbol, time_s::Time, time_e::Time, date_s::Date, date_e::Date) = Span(s_id, imm, DateTime(date_s, time_s), DateTime(date_e, time_e))

Base.isless(a::Span, b::Span) = a.s < b.s ? true : (a.s == b.s ? a.e < b.e : false) # arbitrary order

function expand(s_id::Int, imm::Symbol, time_s::String, time_e::String, date_s::String, date_e::String, dow::Symbol)
    # At the time this is called
    sp = Span[]
    any([time_s, time_e, date_s, date_e] .== "") && return sp
    dow == Symbol("") && return sp
    try
        sp = expand(s_id, imm::Symbol, Time(time_s), Time(time_e),Date(date_s) ,Date(date_e), daytoid[dow])
    catch e
...
function expand(s_id::Int, imm::Symbol, time_s::Time, time_e::Time, date_s::Date, date_e::Date, dow_id::Int)
    date_s = tonext(date_s, dow_id, same=true)
    date_e = toprev(date_e, dow_id, same=true)
    return [Span(s_id, imm, time_s, time_e, d, d) for d in date_s:Week(1):date_e]
end
...
function Base.show(io::IO, span::Span)
    ...
    print(io, "($(span.s_id)) $d [$(idtoday[dw]) $s-$e] @ $(string(span.imm))")
end
...
_conflict(a::Vector{Span}, b::Vector{Span}, d::Minute) = any([_conflict(sa, sb, d) for sa in a, sb in b])

conflict_expl(a::Vector{Span}, b::Vector{Span}, d::Minute) = [(sa, sb) for sa in a, sb in b if _conflict(sa, sb, d)]
